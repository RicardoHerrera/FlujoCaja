...
- (NSDateFormatter *)utcDateFormatter
{
    if (!_utcDateFormatter) {
        ...
        [_utcDateFormatter setLocale:[[NSLocale alloc] initWithLocaleIdentifier:@"en_US_POSIX"]];
        [_utcDateFormatter setDateFormat:@"yyyy-MM-dd'T'H:mm:ss'Z'"];
    }
...
- (NSInteger) getNumberOfDaysOfCurrentMonth
{
    NSRange days = [CURRENT_CALENDAR rangeOfUnit:NSDayCalendarUnit inUnit:NSMonthCalendarUnit forDate:self];
    return days.length;
}
...
{
    NSInteger daysOfMonth = [self getNumberOfDaysOfCurrentMonth];
    return [self dateByAddingDays:daysOfMonth-self.day];
}
...
{
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init] ;
    return [[dateFormatter weekdaySymbols] objectAtIndex:(self.weekday-1)];
}
...
- (NSString *)utcString
{
    NSString *utcDateString = [self.utcDateFormatter stringFromDate:self];
